...
##   messages that overwrite eachother, error messages with trace strings when
...
##   Save this file with another name so you don't overwrite the template.
...
##       desired format.
##     Edit the help and usage subroutines at the bottom of this script.
...
##     Other common editing locations are commented with all-caps, such as:
##       ENTER YOUR COMMAND LINE PARAMETERS HERE AS BELOW
...
##     together.  It will even asociate a common individual file with all the
##     files of another type.  See the comments above the subroutine itself for
##     examples of the combinations this subroutine can handle.  Use in concert
##     with sglob, given files that are similarly named.
##   transpose
##     This subroutine will transpose a 2 dimensional array.  (Used by
##     getFileSets.)
##   getExistingOutfiles
...
##   mkdirs
##     Creates groups of directories unless in dry run mode.
##   checkFile
##     This subroutine determines whether an unwanted overwrite will occur and
##     (an overwrite is detected) will either exit the script if the overwrite
##     flag was not supplied or will return false if the --skip-existing flag
##     was not supplied.
##   openIn
##     Opens input files.  Returns false if unable to open an input file.
##     Reports messages via verbose and error subroutines.
##   closeIn
##     Closes input files.  Reports messages via verbose and error subroutines.
##   openOut
##     Opens output files unless in dry run mode.  Returns false if unable to
##     open an output file.  Reports messages via verbose and error
##     subroutines.
##   closeOut
##     Closes output files unless in dry run mode.  Reports messages via
