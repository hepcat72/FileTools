...
sub getFileSets
  {
	...
	debug("Input detected on STDIN") if($DEBUG > 99);
	...
	unless(scalar(grep {$_ eq '-'} map {@$_} @$input_files))
	  {
	    debug("Adding STDIN to input files.") if($DEBUG > 99);
	    ...
	    #Else create a new input file set with it as the only file member
	      ...
	      {push(@{$file_types_array->[0]},['-'])}
	  }
      }
    elsif($DEBUG > 99)
      {debug("No input on STDIN detected.")}
...
end_print
    ...
    if(!$advanced)
      {
	print << "end_print";
* WHAT IS THIS: Case-insensitive sort of the lines of a file, treating letters 
                alphabetically and numbers numerically.  Non-alpha-numeric
                characters are sorted based on their ascii values.

* INPUT FORMAT: Any ascii text file.
...
#Copied this subroutine from /Users/rwleach/robleach/pub/Sub_RealSmartSort.pl
#on 9/5/2012 and edited to improve it and add comments
#
#  Subroutine:  RealSmartSort
#  Usage:            RealSmartSort(ref to array of strings and numbers)
#  Purpose:        Sorts an array Alpha-Numerically
#  Takes:            Reference to an array containing strings and numbers
#  Returns:        nothing really, but returns the array reference anyway
#
sub RealSmartSort
  {
    my $i;
    my $arrayref = (ref($_[0]) ne 'ARRAY' ? [@_] : [@{$_[0]}]);
    my $highnum = 0;
    my $hash;
    my $key;
    my $integer;
    ...
    #Store the hash of the array values and determine the number of digits in
    #the largest integer value
    foreach(@$arrayref)
      {
	verboseOverMe("Inspecting numbers in string [",++$elem,"].");

	#If this is a commented or empty line
	if(/^\s*#/ || /^\s*$/)
	  {
	    #If we're past the header and have encountered an empty line, skip
	    if($data_found && /^\s*$/)
	      {
	    ...
	    #Assume this is a commented or empty line in a header
	    verbose("Saving header.");
	    push(@header_lines,$_);
	    next;
	  }

	$data_found = 1; #Used for skipping empty lines (not comments, which
	                 #will all go in the header - a more advanced version
	                 #should make any commented lines stick to either the
	                 #line below or above

	#If this is a duplicate line, keep track of duplicates by appending a
	#separator string
	if(exists($hash->{$_}))
	  {
	    $duplicate_seperator = "__<" . $duplicate_index++ . ">__";
	    chomp;
	    $_ = $_ . $duplicate_seperator . "\n";
	  }

	#Store the line in a hash
	$hash->{$_} = uc($_);

	#Isolate all the integer values from the string
	my $tmp = $_;
	$tmp =~ s/(?<![\d\.])(\d*)\.\d+(?!\.\S|\d)/$1/g;
	@integers = ($tmp =~ /(\d+)/g);
	debug("@integers  ");

	#Determine the number of digits in the largest integer value
	foreach $integer (@integers)
	  {if(length($integer) > $highnum)
	     {$highnum = length($integer)}}

	undef(@integers);
      }
    debug("highnum = $highnum");

    verbose("\n");

    $elem = 0;

    #Alter the values in the hash to prepend 0's onto the integer values so
    #that they will be sorted numerically using cmp
    foreach $key (keys(%$hash))
      {
	verboseOverMe("Padding numbers in string [",++$elem,"].");

	debug("Unordered hash:    $hash->{$key}");

	@decimals = ($hash->{$key} =~ /(?<![\d\.])\d*(\.\d+)(?!\d|\.\S)/g);
	$hash->{$key} =~ s/(?<![\d\.])(\d*)\.\d+(?!\d|\.\S)/$1decimallvaluee/g;
	debug("Without Decimals: $hash->{$key}");

	@integers = ($hash->{$key} =~ /(\d+)/g);
	$hash->{$key} =~ s/(\d+)/integerrvaluee/g;
	debug("Without Integers: $hash->{$key}");

	$hash->{$key} = lc "$hash->{$key}";
	debug("Without Uppercase:$hash->{$key}");

	foreach $integer (@integers)
	  {
	    $integer = "0" x ($highnum - length($integer)) . $integer;
	    $hash->{$key} =~ s/integerrvaluee/$integer/;
	    debug("With lead zero:    $hash->{$key}");
	  }
	foreach $decimal (@decimals)
	  {
	    $hash->{$key} =~ s/decimallvaluee/$decimal/;
	    debug("With decimal back:$hash->{$key}");
	  }
	debug("After Edit:            $hash->{$key}");
	undef(@decimals);
	undef(@integers);
      }
    ...
    #Shorten the array by the number of lines that were not added to the hash
    pop(@$arrayref) foreach(1..($num_skipped+scalar(@header_lines)));

    #Sort the hash based on the altered values
    $i    = 0;
    $elem = 0;
    foreach $key (sort {$hash->{$a} cmp $hash->{$b}} keys(%$hash))
      {
	verboseOverMe("String [",++$elem,"] sorted.");

	#Take out the $duplicate_seperator that was appended to identical lines
	$key =~ s/__\<\d+\>__$//;
    ...
    unshift(@$arrayref,@header_lines) if(scalar(@header_lines));
